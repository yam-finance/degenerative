...
export const getTxStats = async (
  ...
  try {
    ...
    const txCount = new BigNumber(txs["txCount"]).toString();
    const failedTxCount = new BigNumber(txs["failedTxCount"]).toString();
    const totalFailedGasFeeETH = new BigNumber(txs["totalFailedGasFeeETH"]).decimalPlaces(3).toString();

    return [totalGasFeeETH, averageTxPriceGWEI, txCount, failedTxCount, totalFailedGasFeeETH];
  } catch (e) {
